...
const csvPath = path.join(repoRoot, 'dados-fonte', 'nilo', 'TitulacaoDocente.csv');
const sqlPath = path.join(repoRoot, 'tmp', 'stg_nilo_titulacao_docente.sql');
const tableName = 'stg_nilo_titulacao_docente';
...
COMMENT ON TABLE staging.${tableName} IS 'Tabela staging para Titulação Docente da Plataforma Nilo Peçanha';
...
COMMENT ON COLUMN staging.${tableName}.servidores_docente_efetivo IS 'Número de docentes efetivos';
COMMENT ON COLUMN staging.${tableName}.servidores_numero_de_docentes IS 'Número total de docentes';
COMMENT ON COLUMN staging.${tableName}.servidores_numero_de_servidores IS 'Número total de servidores';
COMMENT ON COLUMN staging.${tableName}.servidores_itcd IS 'ITCD. TEXT pois usa vírgula como decimal na fonte';
...
async function main() {
  ...
  try {
    ...
    const copySql = `COPY staging.${tableName} (ano, regiao, uf, estado, organizacao_academica_pnp, instituicao, instituicao_nome, servidores_docente_efetivo, servidores_numero_de_docentes, servidores_numero_de_servidores, servidores_itcd) FROM STDIN WITH CSV HEADER DELIMITER ';' ENCODING 'UTF8' NULL ''`;
