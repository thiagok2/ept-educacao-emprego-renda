...
const csvPath = path.join(repoRoot, 'dados-fonte', 'nilo', 'ReservaVagas.csv');
const sqlPath = path.join(repoRoot, 'tmp', 'stg_nilo_reserva_vagas.sql');
const tableName = 'stg_nilo_reserva_vagas';
...
COMMENT ON TABLE staging.${tableName} IS 'Tabela staging para Reserva de Vagas da Plataforma Nilo Peçanha';
...
COMMENT ON COLUMN staging.${tableName}.tipo_reserva_vaga IS 'Tipo de reserva de vaga';
COMMENT ON COLUMN staging.${tableName}.vagas_regulares IS 'Número de vagas regulares';
COMMENT ON COLUMN staging.${tableName}.vagas_regulares_percentual IS 'Percentual de vagas regulares. TEXT pois usa vírgula como decimal na fonte';
...
async function main() {
  ...
  try {
    ...
    const copySql = `COPY staging.${tableName} (ano, regiao, uf, estado, organizacao_academica_pnp, instituicao, instituicao_nome, nome_unidade_recente, tipo_reserva_vaga, vagas_regulares, vagas_regulares_percentual) FROM STDIN WITH CSV HEADER DELIMITER ';' ENCODING 'UTF8' NULL ''`;
