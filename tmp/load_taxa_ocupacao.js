...
const csvPath = path.join(repoRoot, 'dados-fonte', 'nilo', 'TaxaOcupacao.csv');
const sqlPath = path.join(repoRoot, 'tmp', 'stg_nilo_taxa_ocupacao.sql');
const tableName = 'stg_nilo_taxa_ocupacao';
...
COMMENT ON TABLE staging.${tableName} IS 'Tabela staging para Taxa de Ocupação da Plataforma Nilo Peçanha';
...
COMMENT ON COLUMN staging.${tableName}.taxa_ocupacao_matriculas_ciclos_vigentes IS 'Matrículas em ciclos vigentes para cálculo da taxa de ocupação';
COMMENT ON COLUMN staging.${tableName}.taxa_ocupacao_vagas_ciclos_vigentes IS 'Vagas em ciclos vigentes para cálculo da taxa de ocupação';
COMMENT ON COLUMN staging.${tableName}.taxa_ocupacao_taxa_de_ocupacao IS 'Taxa de ocupação. TEXT pois usa vírgula como decimal na fonte';
...
async function main() {
  ...
  try {
    ...
    const copySql = `COPY staging.${tableName} (ano, regiao, uf, estado, organizacao_academica_pnp, instituicao, instituicao_nome, taxa_ocupacao_matriculas_ciclos_vigentes, taxa_ocupacao_vagas_ciclos_vigentes, taxa_ocupacao_taxa_de_ocupacao) FROM STDIN WITH CSV HEADER DELIMITER ';' ENCODING 'UTF8' NULL ''`;
