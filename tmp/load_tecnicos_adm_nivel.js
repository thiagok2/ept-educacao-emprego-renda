...
const csvPath = path.join(repoRoot, 'dados-fonte', 'nilo', 'TecnicosAdmNivel.csv');
const sqlPath = path.join(repoRoot, 'tmp', 'stg_nilo_tecnicos_adm_nivel.sql');
const tableName = 'stg_nilo_tecnicos_adm_nivel';
...
COMMENT ON TABLE staging.${tableName} IS 'Tabela staging para Técnicos Administrativos por Nível da Plataforma Nilo Peçanha';
...
COMMENT ON COLUMN staging.${tableName}.titulacao IS 'Titulação do técnico administrativo';
COMMENT ON COLUMN staging.${tableName}.servidores_numero_de_tae IS 'Número de técnicos administrativos de nível especificado';
...
async function main() {
  ...
  try {
    ...
    const copySql = `COPY staging.${tableName} (ano, regiao, uf, estado, organizacao_academica_pnp, instituicao, instituicao_nome, titulacao, servidores_numero_de_tae) FROM STDIN WITH CSV HEADER DELIMITER ';' ENCODING 'UTF8' NULL ''`;
