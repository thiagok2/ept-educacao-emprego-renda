...
const csvPath = path.join(repoRoot, 'dados-fonte', 'nilo', 'EficienciaAcademica.csv');
const sqlPath = path.join(repoRoot, 'tmp', 'stg_nilo_eficiencia_academica.sql');
const tableName = 'stg_nilo_eficiencia_academica';
...
COMMENT ON TABLE staging.${tableName} IS 'Tabela staging para Eficiência Acadêmica da Plataforma Nilo Peçanha';
...
COMMENT ON COLUMN staging.${tableName}.eficiencia_academica_concluidos IS 'Número de alunos concluídos';
COMMENT ON COLUMN staging.${tableName}.eficiencia_academica_concluidos_percentual IS 'Percentual de concluídos. TEXT pois usa vírgula como decimal na fonte';
COMMENT ON COLUMN staging.${tableName}.eficiencia_academica_indice_eficiencia_academica_percentual IS 'Índice de eficiência acadêmica em percentual. TEXT pois usa vírgula como decimal na fonte';
COMMENT ON COLUMN staging.${tableName}.eficiencia_academica_numero_de_evadidos IS 'Número de evadidos';
COMMENT ON COLUMN staging.${tableName}.eficiencia_academica_retidos IS 'Número de retidos';
COMMENT ON COLUMN staging.${tableName}.eficiencia_academica_retidos_percentual IS 'Percentual de retidos. TEXT pois usa vírgula como decimal na fonte';
COMMENT ON COLUMN staging.${tableName}.eficiencia_academica_taxa_de_evasao_percentual IS 'Taxa de evasão em percentual. TEXT pois usa vírgula como decimal na fonte';
...
async function main() {
  ...
  try {
    ...
    const copySql = `COPY staging.${tableName} (ano, regiao, uf, estado, organizacao_academica_pnp, instituicao, instituicao_nome, nome_unidade_recente, eficiencia_academica_concluidos, eficiencia_academica_concluidos_percentual, eficiencia_academica_indice_eficiencia_academica_percentual, eficiencia_academica_numero_de_evadidos, eficiencia_academica_retidos, eficiencia_academica_retidos_percentual, eficiencia_academica_taxa_de_evasao_percentual) FROM STDIN WITH CSV HEADER DELIMITER ';' ENCODING 'UTF8' NULL ''`;
