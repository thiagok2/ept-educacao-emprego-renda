...
const csvPath = path.join(repoRoot, 'dados-fonte', 'nilo', 'PercentuaisLegais.csv');
const sqlPath = path.join(repoRoot, 'tmp', 'stg_nilo_percentuais_legais.sql');
const tableName = 'stg_nilo_percentuais_legais';
...
COMMENT ON TABLE staging.${tableName} IS 'Tabela staging para Percentuais Legais da Plataforma Nilo Peçanha';
...
COMMENT ON COLUMN staging.${tableName}.matricula_equivalente_formacao_de_professores IS 'Matrícula equivalente para formação de professores. TEXT pois usa vírgula como decimal na fonte';
COMMENT ON COLUMN staging.${tableName}.matricula_equivalente_tecnicos IS 'Matrícula equivalente para técnicos. TEXT pois usa vírgula como decimal na fonte';
COMMENT ON COLUMN staging.${tableName}.matricula_equivalente_proeja IS 'Matrícula equivalente para Proeja. TEXT pois usa vírgula como decimal na fonte';
...
async function main() {
  ...
  try {
    ...
    const copySql = `COPY staging.${tableName} (ano, regiao, uf, estado, organizacao_academica_pnp, instituicao, instituicao_nome, nome_unidade_recente, matricula_equivalente_formacao_de_professores, matricula_equivalente_tecnicos, matricula_equivalente_proeja, matricula_equivalente_geral) FROM STDIN WITH CSV HEADER DELIMITER ';' ENCODING 'UTF8' NULL ''`;
