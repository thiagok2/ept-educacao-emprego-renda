...
const csvPath = path.join(repoRoot, 'dados-fonte', 'nilo', 'RelacaoInscritosVagas.csv');
const sqlPath = path.join(repoRoot, 'tmp', 'stg_nilo_relacao_inscritos_vagas.sql');
const tableName = 'stg_nilo_relacao_inscritos_vagas';
...
COMMENT ON TABLE staging.${tableName} IS 'Tabela staging para Relação Inscritos x Vagas da Plataforma Nilo Peçanha';
...
COMMENT ON COLUMN staging.${tableName}.numero_de_inscritos IS 'Número de inscritos';
COMMENT ON COLUMN staging.${tableName}.numero_de_vagas IS 'Número de vagas';
COMMENT ON COLUMN staging.${tableName}.relacao_inscrito_vaga IS 'Relação inscrito/vaga. TEXT pois usa vírgula como decimal na fonte';
...
async function main() {
  ...
  try {
    ...
    const copySql = `COPY staging.${tableName} (ano, regiao, uf, estado, organizacao_academica_pnp, instituicao, instituicao_nome, nome_unidade_recente, numero_de_inscritos, numero_de_vagas, relacao_inscrito_vaga) FROM STDIN WITH CSV HEADER DELIMITER ';' ENCODING 'UTF8' NULL ''`;
