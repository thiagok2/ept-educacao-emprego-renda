...
const csvPath = path.join(repoRoot, 'dados-fonte', 'nilo', 'ProfessoresPorInstituicao.csv');
const sqlPath = path.join(repoRoot, 'tmp', 'stg_nilo_professores_por_instituicao.sql');
const tableName = 'stg_nilo_professores_por_instituicao';
...
COMMENT ON TABLE staging.${tableName} IS 'Tabela staging para Professores por Instituição da Plataforma Nilo Peçanha';
...
COMMENT ON COLUMN staging.${tableName}.titulacao IS 'Titulação do professor';
COMMENT ON COLUMN staging.${tableName}.jornada_de_trabalho IS 'Jornada de trabalho do docente';
COMMENT ON COLUMN staging.${tableName}.servidores_numero_de_docentes IS 'Número de docentes';
...
async function main() {
  ...
  try {
    ...
    const copySql = `COPY staging.${tableName} (ano, regiao, uf, estado, organizacao_academica_pnp, instituicao, instituicao_nome, titulacao, jornada_de_trabalho, servidores_numero_de_docentes) FROM STDIN WITH CSV HEADER DELIMITER ';' ENCODING 'UTF8' NULL ''`;
