...
async function run() {
  ...
  for (const t of tables) {
    ...
    `, [name]);
    const uniques = uniqRes.rows.map(r=>r.cols);
    ...
    // row count
    const countRes = await client.query(`SELECT COUNT(*) AS cnt FROM public."${name}"`);
    ...
    for (const c of criticalCols) {
      const q = `SELECT COUNT(*) AS cnt FROM public."${name}" WHERE "${c.column_name}" IS NULL`;
    ...
    for (const [ck, info] of Object.entries(fkGroups)) {
      ...
      const sql = `SELECT COUNT(*) AS cnt FROM public."${name}" t LEFT JOIN ${info.ref_schema}."${info.ref_table}" r ON ${joinCond} WHERE (${info.columns.map(c=>`t."${c}" IS NOT NULL`).join(' OR ')}) AND r."${info.ref_columns[0]}" IS NULL`;
      try {
    ...
    let fatoCount = null;
    let fatoTable = null;
    for (const schema of ['staging','public']) {
      try {
        const r = await client.query(`SELECT COUNT(*) AS cnt FROM ${schema}."${possibleFato}"`);
        fatoCount = parseInt(r.rows[0].cnt,10);
        fatoTable = `${schema}.${possibleFato}`;
        break;
      } catch (e) {
        // ignore
      }
    ...
    if (hasCo) {
      // try identify column name
      const candidates = columns.filter(c=>/co_esfera|relacao_do_orgao|orgao_uge/i.test(c.column_name)).map(c=>c.column_name);
      for (const col of candidates) {
        const rr = await client.query(`SELECT COUNT(*) AS cnt FROM public."${name}" WHERE "${col}" IS NOT NULL AND "${col}" != 'UGE'`);
    ...
    if (uniques.length>0) {
      ...
      const dupQ = `SELECT COUNT(*) AS dup_count FROM (SELECT ${qCols}, COUNT(*) AS c FROM public."${name}" GROUP BY ${qCols} HAVING COUNT(*)>1) x`;
      try {
