...
const csvPath = path.join(repoRoot, 'dados-fonte', 'nilo', 'RelacaoAlunoProfessorRAP.csv');
const sqlPath = path.join(repoRoot, 'tmp', 'stg_nilo_relacao_aluno_professor_rap.sql');
const tableName = 'stg_nilo_relacao_aluno_professor_rap';
...
COMMENT ON TABLE staging.${tableName} IS 'Tabela staging para Relação Aluno Professor RAP da Plataforma Nilo Peçanha';
...
COMMENT ON COLUMN staging.${tableName}.rap_rap IS 'RAP. TEXT pois usa vírgula como decimal na fonte';
COMMENT ON COLUMN staging.${tableName}.rap_matriculas_rap IS 'Número de matrículas RAP';
COMMENT ON COLUMN staging.${tableName}.rap_professor_equivalente IS 'Professor equivalente RAP';
...
async function main() {
  ...
  try {
    ...
    const copySql = `COPY staging.${tableName} (ano, regiao, uf, estado, organizacao_academica_pnp, instituicao, instituicao_nome, nome_unidade_recente, rap_rap, rap_matriculas_rap, rap_professor_equivalente) FROM STDIN WITH CSV HEADER DELIMITER ';' ENCODING 'UTF8' NULL ''`;
