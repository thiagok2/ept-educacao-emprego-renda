...
const csvPath = path.join(repoRoot, 'dados-fonte', 'nilo', 'CargosCarreira.csv');
const sqlPath = path.join(repoRoot, 'tmp', 'stg_nilo_cargos_carreira.sql');
...
const tableName = 'stg_nilo_cargos_carreira';
...
COMMENT ON TABLE staging.${tableName} IS 'Tabela staging para Cargos e Carreiras da Plataforma Nilo Peçanha';
...
COMMENT ON COLUMN staging.${tableName}.carreira_sigla IS 'Sigla da carreira ou categoria de servidor';
COMMENT ON COLUMN staging.${tableName}.numero_de_servidores_siafi IS 'Número de servidores segundo SIAFI';
...
async function main() {
  ...
  try {
    ...
    const copySql = `COPY staging.${tableName} (ano, regiao, uf, estado, organizacao_academica_pnp, instituicao, instituicao_nome, carreira_sigla, numero_de_servidores_siafi) FROM STDIN WITH CSV HEADER DELIMITER ';' ENCODING 'UTF8' NULL ''`;
